...
@implementation RandomValues

+(id)create {
    return [[RandomValues alloc] init];
}
...
-(double)nextDoubleMax:(double) maxExclusive {
    return [self next] * maxExclusive / 23328.0;
}

-(int)nextIntMax:(int) maxExclusive {
    return [self next] * maxExclusive / 23328;
}

NSString const *characters = @"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxy0123456789";
...
-(double)nextDouble {
    return [self nextDoubleMax:18446744073709552000.0];
}

-(BOOL)nextBoolean {
    return [self nextIntMax:2]==1;
}
...
-(id)nextFromArray:(NSArray*)arr {
    return [arr objectAtIndex:[self nextIntMax:[arr count]]];
}

-(NSString*)nextTextWord {
    return [self nextFromArray:testArray];
}

-(NSString*)nextString:(int)length {
    NSMutableString *s = [NSMutableString stringWithCapacity:length];
    for (int i = 0; i < length; i++) {
        [s appendFormat:@"%C", [characters characterAtIndex:[self nextIntMax:[characters length]]]];
    }
    return s;
}

-(NSString*)nextTextWithWords:(int)countWords {
    NSMutableString *s = [NSMutableString stringWithCapacity:countWords*4];
    for (int i = 0; i < countWords; i++) {
        [s appendFormat:@"%@ ", [self nextTextWord]];
    }
    return s;
}
