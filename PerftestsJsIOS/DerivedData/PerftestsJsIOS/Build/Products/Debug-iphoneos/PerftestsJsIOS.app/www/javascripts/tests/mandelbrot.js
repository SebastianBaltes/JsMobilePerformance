module('perftestsjs.tests').mandelbrot = (function() {
	...
	function MandelbrotCalculator(width, height) {
			...
			for ( var x = 0; x < this.width; x++) {
				for ( var y = 0; y < this.height; y++) {
					...
					if (this.iterationPixels[i] == 0) {
						...
						var f = Math.abs(Math.sin(1 / ((Math.PI + 0.7) * c
								.abs())));
		...
		this.zoom = function(point, zoomFactor) {
			...
			this.o = new Complex(-center.r + this.d.r / 2, center.i + this.d.i
					/ 2);
			for ( var x = 0; x < this.width; x++) {
				for ( var y = 0; y < this.height; y++) {
					var i = x + y * this.width;
		...
		this.rgbToInt = function(r, g, b) {
			return ((255 & 0xFF) << 24) | ((r & 0xFF) << 16)
					| ((g & 0xFF) << 8) | ((b & 0xFF) << 0);
		};

		this.toGraphCoord = function(point) {
			return new Complex(point.x * this.d.r / this.width - this.o.r,
					this.o.i - point.y * this.d.i / this.height);
		};
		...
		this.Hsb2Rgb = function(hue, saturation, brightness) {
			...
			return 0xff000000 | (Math.floor(r) << 16) | (Math.floor(g) << 8)
					| (Math.floor(b) << 0);
		};
	...
	function startFN() {
		...
		var framesPerSecond = $('#MandelbrotPage #framesPerSecond');

		var canvas = $('#MandelbrotPage #canvas').get()[0];
		...
		var mandelbrot = new MandelbrotCalculator(mandelbrotSide,
				mandelbrotSide);

		var unscaledCanvas = $("<canvas>").attr("width", image.width).attr(
		...
		$('#MandelbrotPage>#canvas').click(
				function(e) {
					var x = (e.pageX - $(this).position().left)
							/ $(this).width() * mandelbrotSide;
					var y = (e.pageY - $(this).position().top)
							/ $(this).height() * mandelbrotSide;
